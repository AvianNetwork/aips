...
1. **Time source:** compute epoch index `T = floor((MTP(parent_of_spend_block) − t0) / step)` where MTP is the Median‑Time‑Past of the parent block (BIP113‑style).
1. **Window:** for `w ∈ {−skew … +skew}`, compute code `C_w` per RFC‑6238 (HOTP truncate) with HMAC(`algo`, `K_i`, `T+w`) and accept if any match.
1. **Result:** push `1` on success, else `0`.
...
## Diagram
[center|700px](File:aip-0006/flightlock_diagram.png)
