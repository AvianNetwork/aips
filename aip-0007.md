...
1. **parentHashes[] (32‑byte each)** — ordered list of parents; the first is a miner‑hinted preferred parent.
1. **blueScore (uint64)** — cumulative blue score up to and including this block.
1. **blueWork (uint256)** — cumulative PoW over the flock set (big‑endian).
1. **daaScore (uint64)** — index in virtual order for DAA computations.
1. **pruningPoint (32‑byte)** — hash of current pruning point anchor (optional in Phase 1; mandatory in Phase 2).
1. **algoId (uint8)** — identifier of the PoW algorithm used (e.g., 0 = X16RT, 1 = MinotaurX).
...
1. All parents exist and timestamps are sane (median‑of‑parents + drift guard).
1. GhostDAG calculation places the block and derives (blueScore, blueWork) deterministically.
1. The block’s declared (blueScore, blueWork) must match locally computed values.
1. Virtual order updates exactly once per accepted block set; ties broken lexicographically by (blueWork, hash).
1. DAA target for the block matches computed target at its `daaScore` position.
1. Pruning rules (once enabled) forbid references older than the pruning point.
1. Standard tx, scripts, and mempool rules remain unchanged.
